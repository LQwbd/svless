...
let userID = '6e1fc555-7aec-4d49-9056-5ca9ba9ccd04';

let proxyIP = '';// 小白勿动，该地址并不影响你的网速，这是给CF代理使用的'cdn.xn--b6gac.eu.org, cdn-all.xn--b6gac.eu.org, workers.cloudflare.cyou'

//let sub = '';// 留空则显示原版内容
let sub = 'w2.qingdeng.dns-dynamic.net';// 内置优选订阅生成器，可自行搭建 https://github.com/LQwbd/sdyq
let subconverter = 'SUBAPI.fxxk.dedyn.io';// clash订阅转换后端。自带虚假uuid和host订阅。
let subconfig = "https://raw.githubusercontent.com/LQwbd/ACL4SSR/main/Clash/config/ACL4SSR_Online_Full_MultiMode.ini"; //订阅配置文件
				...
				case `/${userID}`: {
					...
					if (userAgent && userAgent.includes('mozilla')){
						...
						return new Response(`${vlessConfig}`, {
							status: 200,
							headers: {
								"Content-Disposition": "attachment; filename=青灯; filename*=utf-8''青灯",
								"Content-Type": "text/plain;charset=utf-8",
								"Profile-Update-Interval": "6",
								"Subscription-Userinfo": `qingdeng`,
							}
						});
					}
				}
...
function 配置信息(UUID, 域名地址) {
	const 落霞与孤鹜齐飞_秋水共长天一色 = 'dmxlc3M=';
	const 协议类型 = atob(落霞与孤鹜齐飞_秋水共长天一色);
	...
	const v2ray = `${协议类型}://${用户ID}@${地址}:${端口}?加密方式=${加密方式}&security=${传输层安全[0]}&sni=${SNI}&fp=${指纹}&type=${传输层协议}&host=${伪装域名}&path=${encodeURIComponent(路径)}#${encodeURIComponent(别名)}`;
...
async function getVLESSConfig(userID, hostName, sub, userAgent, RproxyIP) {
	...
	`;
		...
		try {
			...
			headers: {
				'User-Agent': 'CF-Workers-LQLY/LQwbd'
			}});
